...
- **OP-UR, the overlap region**: `(∪_d unlearn_topk_d) ∩ (∪_d retention_topk_d)`,
  the weights important both for forgetting and for retention.
- **OP-RR, the shared retention core**: `∩_d retention_topk_d`,
  the weights important for retaining every domain.
...
Training uses an ascent-plus-descent objective (forget samples are pushed up, retain samples are
pulled down), with each sample tagged by a 4-way `domain x segment` factor
(`llm_unlearn/utils/grail_tokenizer.py`: privacy/copyright x unlearn/retention).
...
environment). Run the stages in order, each consuming the artifact produced by the previous one:
...
bash scripts/1_compute_gradients.sh   # produces grad_info_{domain}_{split}_3.pt (per-parameter gradients)
bash scripts/2_localize_grail.sh      # produces UR_top<k>.pt, RR_top<k>.pt (element masks)
bash scripts/3_tokenize.sh            # produces tokenized_dataset.pt (4-factor combined set)
bash scripts/4_unlearn_grail.sh       # produces unlearned checkpoints
CKPT=<checkpoint dir> bash scripts/5_evaluate.sh
```

**Artifact layout.** Stages 1 and 2 write under
...
`RR_percent`, and `localized_type` values must match between stages 2 and 4.
...
- **Forget success** = `100 - accuracy` on the forget set (higher means more forgotten)
- **Retention success** = `accuracy` on the retain set (higher means more retained)
