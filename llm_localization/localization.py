...
# data_module lives in the repo's pretrain/ directory (sibling of llm_localization/)
...
class DataTrainingArguments:
    ...
    ######## set \mu and \sigma
    # LLaMA-2-chat
    # Copyright \mu: 0.92, \sigma: 6e-4
    # Privacy \mu: 0.96, \sigma: 4e-4
    # Combined \mu: 0.94, \sigma: 5e-4

    sim_thresh: float = field(
        default=0.92,
        metadata={
            "help": "\mu in the paper."
        },
    )
    grad_thresh: float = field(
        default=6e-4,
        metadata={
    ...
    data_type: str = field(
        default="",
        metadata={"help": "Choose training task.", "choices": ("privacy", "copyright", "combined"),}
    )
    ...
    save_tag: Optional[str] = field(
        default="",
        metadata={"help": "tag for save folder"},
    )
...
def main(model_args, data_args):
    ...
    model_version = model_args.model_name_or_path.split('/')[-1]
    ...
    for split in data_args.splits.split(","):
        if os.path.exists(f"outputs/{model_name}/{model_version}/grad_info_{data_args.data_type}_{split}_{num_copies}.pt"):
            continue
        ...
        torch_format_dataset = TextDatasetRandomQA(
            "zjunlp/KnowUnDo", 
            tokenizer=tokenizer, 
        ...
        if data_args.save_tag == "":
            if not os.path.exists(f"outputs/{model_name}/{model_version}"):
                os.makedirs(f"outputs/{model_name}/{model_version}", exist_ok=True)
            # save info_matrix
            torch.save(info_matrix, f"outputs/{model_name}/{model_version}/grad_info_{data_args.data_type}_{split}_{num_copies}.pt")
        else:
            if not os.path.exists(f"outputs/{model_name}/{data_args.save_tag}/{model_version}"):
                os.makedirs(f"outputs/{model_name}/{data_args.save_tag}/{model_version}", exist_ok=True)
            # save info_matrix
            torch.save(info_matrix, f"outputs/{model_name}/{data_args.save_tag}/{model_version}/grad_info_{data_args.data_type}_{split}_{num_copies}.pt")
    
    if data_args.save_tag == "":
        grad_retention = torch.load(f'outputs/{model_name}/{model_version}/grad_info_{data_args.data_type}_retention_{num_copies}.pt')
        grad_unlearn = torch.load(f'outputs/{model_name}/{model_version}/grad_info_{data_args.data_type}_unlearn_{num_copies}.pt')

    else:
        grad_retention = torch.load(f'outputs/{model_name}/{data_args.save_tag}/{model_version}/grad_info_{data_args.data_type}_retention_{num_copies}.pt')
        grad_unlearn = torch.load(f'outputs/{model_name}/{data_args.save_tag}/{model_version}/grad_info_{data_args.data_type}_unlearn_{num_copies}.pt')
    ...
    # grad_unlearn 과 grad_retention의 abs, norm 계산
    ...
    if data_args.save_tag == "":
        with open(f"outputs/{model_name}/{model_version}/located_region_{data_args.data_type}.json", "w") as f:
            json.dump(located_region, f, indent=4)
    else:
        with open(f"outputs/{model_name}/{data_args.save_tag}/{model_version}/located_region_{data_args.data_type}.json", "w") as f:
            json.dump(located_region, f, indent=4)
